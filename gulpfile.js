...
function copyVideo(cb) {
  let videoPath = `${dir.src}video/`;
  if(fileExist(videoPath)) {
    return src(videoPath + '**/*.*')
...
function serve() {
  ...
  // Видео: все события
  watch([`${dir.blocks}**/video/*.{mp4}`], { events: ['all'], delay: 100 }, series(copyVideo, reload));
...
exports.build = series(
  ...
  parallel(copyImg, copyVideo, writeSassImportsFile, writeJsRequiresFile),
  parallel(copyFonts),
  parallel(compileSass, buildJs),
);
